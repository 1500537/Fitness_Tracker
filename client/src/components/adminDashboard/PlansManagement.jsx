...
import { pricingData as initialPricingData } from '../../assets/assets';
import CustomPopUp from './CustomPopUp';

const PlansManagement = () => {
  const [plans, setPlans] = useState(initialPricingData);
  ...
  const handleAction = () => {
    ...
    if (type === 'add') {
      ...
      setPlans(plans.filter(p => p.id !== data.id));
    }
  ...
  const openModal = (type, data = null) => {
    setModal({
      ...
      type,
      data: data ? { ...data } : { 
        name: '', tagline: '', monthlyPrice: 0, annualPrice: 0, 
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-[#FF7222] selection:text-black overflow-x-hidden relative">
      ...
      {/* --- THEME BACKGROUND ELEMENTS (PROFESSIONAL ADDITION) --- */}
      ...
      <div className="relative z-10 p-4 sm:p-8 lg:p-12 lg:pl-[340px] pt-24 sm:pt-32">
        ...
        {/* --- PRICING GRID (RESPONSIVE & ATTRACTIVE) --- */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 items-stretch">
          <AnimatePresence mode="popLayout">
            {plans.map((plan) => (
              ...
              >
                <div className={`relative h-full rounded-[2.85rem] p-8 sm:p-12 flex flex-col overflow-hidden backdrop-blur-xl ${plan.style}`}>
                  ...
                  <div className="space-y-5 flex-1 mb-12">
                    ...
                    {plan.features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-4 group/item">
                        <div className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center border transition-all ${plan.name === 'Elite Force' ? 'border-white/20 bg-white/5 text-[#FF7222]' : 'border-black/10 bg-black/5 text-black'}`}>
                  ...
                  <button className={`w-full py-6 rounded-[2rem] font-[1000] uppercase text-[11px] tracking-[0.4em] transition-all hover:tracking-[0.5em] active:scale-95 shadow-2xl ${plan.buttonStyle}`}>
                    DEPLOY_PROTOCOL
                  </button>
                </div>
              </motion.div>
        ...
        {/* --- MODAL SYSTEM (RETAINED LOGIC, ENHANCED UI) --- */}
        <CustomPopUp
          isOpen={modal.isOpen}
